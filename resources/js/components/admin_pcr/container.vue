<template>
  <div>
    <v-snackbar text transition="scale-transition" top :color="snackbar.color" v-model="snackbar.show" timeout="3000">
      {{ snackbar.message }}
    </v-snackbar>
    ...
    <v-card outlined>
      <v-card-title>ADMINISTRADOR PCR</v-card-title>
      <v-data-table :headers="headers" :items="fichas" :loading="loading_table"
                    :items-per-page="15" hide-default-footer :page.sync="getCurrentPage">
        ...
        <template v-slot:item.tipo="{item}">
          <template v-if="item.pcr_prueba_molecular">
            <small v-if="item.pcr_prueba_molecular.tipo === 1">CERRO VERDE</small>
            <small v-else-if="item.pcr_prueba_molecular.tipo === 2">PARTICULARES</small>
        ...
        <template v-slot:item.foto="{item}">
          <template v-if="item.pcr_prueba_molecular.resultado === 1">
            <template v-if="item.pcr_prueba_molecular.pcr_foto_muestra">
              ...
              <v-btn @click="eliminarFoto(item.pcr_prueba_molecular.pcr_foto_muestra.id)" small icon><v-icon small>mdi-delete</v-icon></v-btn>
            </template>
            <template v-else>
              <v-btn @click="uploadPhoto(item)" icon small><v-icon small>mdi-camera</v-icon></v-btn>
            </template>
          </template>
        </template>
        <template v-slot:item.actions="{item}">
          <v-btn icon small @click="accionesEditarPM(item)">
            <v-icon small>mdi-pencil</v-icon>
          </v-btn>
          <v-btn v-if="item.pcr_prueba_molecular && item.pcr_prueba_molecular.resultado === null" icon small
                 @click="accionesEliminarPM(item)"><v-icon small>mdi-delete</v-icon>
          </v-btn>
        </template>
      </v-data-table>
        ...
        <v-card>
          ...
          <v-card-text>
            <validation-observer ref="observer" v-slot="{validate}">
              <v-text-field prepend-icon="mdi-account" label="PACIENTE" disabled v-model="form.nom_completo"></v-text-field>
              <v-text-field prepend-icon="mdi-domain" label="EMPRESA" disabled v-model="form.empresa"></v-text-field>
              <validation-provider v-slot="{errors}" name="price" rules="required|double">
                <v-text-field messages="El precio debe de contener decimales" clearable :error-messages="errors"
                              prepend-icon="mdi-currency-usd" label="PRECIO" v-model="form.precio"></v-text-field>
              </validation-provider>
              <validation-provider v-slot="{errors}" name="type" rules="required|integer">
                <v-select v-model="form.tipo" :items="tipos" item-text="descripcion" item-value="idtipo_prueba_molecular"
                          prepend-icon="mdi-test-tube-empty" @change="setPrecio" :error-messages="errors" label="TIPO"></v-select>
              </validation-provider>
              <v-select
              ...
              <v-text-field
                  ...
                  label="COMENTARIO"
                  v-model="form.detalle"></v-text-field>
            </validation-observer>
            <v-checkbox v-model="form.enviar_mensaje" label="Enviar Mensaje de Whatsapp"></v-checkbox>
          </v-card-text>
          <v-divider></v-divider>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn text color="normal" @click="dialog.edit = false">CANCELAR</v-btn>
            <v-btn :loading="loading" color="primary" @click="editPM">GUARDAR</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Nombres', value: 'paciente_isos.full_name', sortable: false },
  ...
  methods: {
    ...
    accionesEliminarPM(ficha) {
      let r = confirm(`Esta seguro de eliminar la prueba molecular del paciente ${ficha.paciente_isos.nom_completo}?, al eliminar se borrarán las fotos de la ficha de investigación y trasansaction id`);
      if (r) this.$store.dispatch('admin_pcr/deletePM', ficha.pcr_prueba_molecular.idpcr_pruebas_moleculares);
    },
    ...
    verFoto(ruta){
      this.images = [];
      this.images.push(`api/v1/fotoPcr/${ruta}`);
      this.$viewer.show();
    },
    inited (viewer) {
      this.$viewer = viewer
